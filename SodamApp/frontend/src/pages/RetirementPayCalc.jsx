...
import { Printer, ArrowLeft, Edit3, Save, Calculator, RefreshCw } from 'lucide-react';
import api from '../api';
...
export default function RetirementPayCalc() {
    ...
    useEffect(() => {
        const fetchStaffs = async () => {
            try {
                // Fetch all staff members including resigned ones
                const res = await api.get('/hr/staff?status=all');
    ...
    // 2. Fetch Initial Calculation Data when Staff changes
    useEffect(() => {
        ...
        const fetchCalcDetail = async () => {
            setLoading(true);
            try {
                // Pass a specific end_date if we have one modified by user?
                // For now, let the backend use staff.end_date or today.
                const res = await api.get(`/hr/retirement/calc/${selectedStaffId}`);
                ...
                // Keep the exact days from backend initially
                setResults({
                ...
                console.error(error);
                alert('퇴직금 산정 기초 데이터를 불러오지 못했습니다.');
            } finally {
                setLoading(false);
            }
        };
    ...
    // 3. Client-side Recalculation based on manual edits
    const recalculate = () => {
        if (!formData.start_date || !formData.end_date) return;
        
        // Re-calculate work days
        ...
        // Final math
        const finalDays = total_days > 0 ? total_days : 90; // fallback
    ...
    if (mode === 'print') {
        ...
        return (
            <div className="bg-slate-50 min-h-screen pb-20">
                ...
                <style jsx="true" global="true">{`@media print { @page { margin: 10mm; size: A4 portrait; } body { background: white; -webkit-print-color-adjust: exact; print-color-adjust: exact; } }`}</style>
            </div>
        );
    }
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow mb-6 card-animate">
                    ...
                    <select 
                        ...
                        onChange={(e) => {
                            ...
                            navigate(e.target.value ? `/retirement-calc/${e.target.value}` : `/retirement-calc`, { replace: true });
                        }}
                        className="w-full max-w-sm p-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none"
                    >
                        <option value="">직원을 선택하세요...</option>
                        {staffList.map(s => (
                            <option key={s.id} value={s.id}>{s.name} ({s.department || '부서없음'})</option>
                        ))}
                    </select>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-indigo-50 border border-indigo-100 p-6 rounded-2xl shadow-sm text-center">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 border-t border-indigo-200/50 pt-6">
                                ...
                                <div className="col-span-2 bg-gradient-to-r from-indigo-500 to-blue-600 p-4 rounded-xl shadow-md text-white flex justify-between items-center group">
