        ...
        <div className="space-y-4">
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 card-animate flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button className="flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-amber-600 text-white text-xs font-bold px-4 py-2 rounded-xl border-none cursor-pointer shadow-md shadow-amber-500/20 hover:shadow-lg hover:shadow-amber-500/30 transition-all" onClick={openAddModal}>
                        <Plus size={16} /> {isHouseholdMode ? '생활비 추가' : '비용 추가'}
                    </button>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-2xl p-10 shadow-sm border border-slate-100 text-center card-animate">
                    ...
                    <h3 className="text-base font-bold text-slate-700 mb-1">{isHouseholdMode ? '개인 생활비 내역이 없습니다' : '사업용 비용 내역이 없습니다'}</h3>
                    <p className="text-sm text-slate-400 mb-2">{year}년 {month}월</p>
                    <p className="text-xs text-slate-400">
                        우측 상단 <strong className="text-slate-600">+ 비용 추가</strong> 또는 <strong className="text-slate-600">업로드</strong> 탭에서 데이터를 등록하세요
                    </p>
                </div>
            ) : (
