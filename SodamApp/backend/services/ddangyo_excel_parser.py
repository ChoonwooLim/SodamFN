...
def extract_summary(rows: list[list]) -> ParsedDdangyoMonth:
    """행 리스트(list-of-lists)에서 (A)/(D)/(E) 요약 추출 — 순수 함수(테스트용)."""
    for i, row in enumerate(rows):
        joined = " ".join("" if c is None else str(c) for c in row)
        if "(A)주문결제" in joined:
            # 같은 행에서 A/D/E 컬럼 인덱스, 값은 다음 행 같은 인덱스
    ...
    raise DdangyoExcelError("요약 라벨 '(A)주문결제' 를 찾지 못했습니다.")
