...
@router.get("/plans")
def list_plans(admin: User = Depends(get_superadmin_user), s: Session = Depends(get_session)):
    """요금제 목록 조회"""
...
def list_global_announcements(admin: User = Depends(get_superadmin_user), s: Session = Depends(get_session)):
    """전체 공지사항 목록"""
...
def list_businesses_dropdown(admin: User = Depends(get_superadmin_user), s: Session = Depends(get_session)):
    """사업장 전환 드롭다운용 경량 목록"""
    businesses = s.exec(
