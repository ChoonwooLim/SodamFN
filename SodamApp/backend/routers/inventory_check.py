...
def get_inventory_items(bid = Depends(get_bid_from_token), session: Session = Depends(get_session)):
    """모든 재고 체크 항목 목록"""
...
def create_inventory_item(data: InventoryItemCreate, bid = Depends(get_bid_from_token), session: Session = Depends(get_session)):
    """재고 체크 항목 추가"""
...
def update_inventory_item(item_id: int, data: InventoryItemUpdate, session: Session = Depends(get_session)):
    """재고 체크 항목 수정"""
...
def seed_default_items(bid = Depends(get_bid_from_token), session: Session = Depends(get_session)):
...
def create_inventory_check(data: InventoryCheckCreate, staff_id: int = 0, staff_name: str = "", bid = Depends(get_bid_from_token), session: Session = Depends(get_session)):
    """직원이 오픈 재고 체크를 등록"""
...
def get_today_inventory(bid = Depends(get_bid_from_token), session: Session = Depends(get_session)):
    """오늘의 재고 체크 목록"""
    today = datetime.date.today()
...
def get_inventory_history(days: int = 7, bid = Depends(get_bid_from_token), session: Session = Depends(get_session)):
    """최근 N일 재고 체크 이력"""
...
def get_inventory_by_date(date_str: str, bid = Depends(get_bid_from_token), session: Session = Depends(get_session)):
    """특정 날짜의 재고 체크"""
    try:
