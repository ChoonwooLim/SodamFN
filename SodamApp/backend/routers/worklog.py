...
def get_worklog(log_id: int, admin: User = Depends(get_superadmin_user), s: Session = Depends(get_session)):
    """작업일지 단건 조회"""
...
@router.post("/worklog")
def create_worklog(data: WorkLogCreate, admin: User = Depends(get_superadmin_user), s: Session = Depends(get_session)):
    """새 작업일지 생성"""
    log = DevWorkLog(
        date=data.date,
        title=data.title,
...
def update_worklog(log_id: int, data: WorkLogUpdate, admin: User = Depends(get_superadmin_user), s: Session = Depends(get_session)):
    """작업일지 수정"""
    log = s.get(DevWorkLog, log_id)
    if not log:
        raise HTTPException(status_code=404, detail="작업일지를 찾을 수 없습니다.")
...
def delete_worklog(log_id: int, admin: User = Depends(get_superadmin_user), s: Session = Depends(get_session)):
    """작업일지 삭제"""
    log = s.get(DevWorkLog, log_id)
    if not log:
        raise HTTPException(status_code=404, detail="작업일지를 찾을 수 없습니다.")
