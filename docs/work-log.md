...
| 카테고리 | 작업 내용 | 상태 |
|----------|----------|------|
| fix | 감가상각비 50% 정정 — 자산대장 취득원가 절반 수정 (사장님 옵션3 선택) | 완료 |
| fix | P/L sync 사업장 교차 오염 버그 전면 차단 (6개 함수 bid=None fan-out 가드) + 오염 데이터 수리 | 완료 |
| feat | 손익계산서 반기·분기 모니터링 + 정식 결산보고서 리디자인 | 완료 |

### 세부 내용

- **감가상각 50% 정정**(3dcd8e02): 사장님 지시로 3가지 방식(잔존가치50%/내용연수2배/취득원가정정) 비교표 제시 → 취득원가 절반 정정 선택. 주방집기·인테리어 5,000만→2,500만, 추가집기 3,000만→1,500만 (대장 note에 이력 보존). 2025-01~2026-07 백필: 월 2,166,666→1,083,334, 2026-05~ 500,000→250,000. 재무상태표 유형자산 취득원가 1.3억→6,500만, notes 하드코딩 제거(자산대장 동적 생성).
- **교차 오염 버그**(3dcd8e02+68d8c990): sync 함수들이 business_id=None(슈퍼어드민 조회)이면 전 사업장 원천 합산값을 임의 사업장 P/L(.first())에 기록. 스크린샷 검증 중 수치 변동으로 실측 포착. 6개 sync 전부에 사업장별 fan-out 가드. 수리: 소담김밥 재동기화(1~6월 기준값 완전 일치 검증), 장인김밥 시드 재생성(결정적), 강동점(원천 0건) 오염 필드 0 초기화. ⚠️ 슈퍼어드민 프론트 표시 혼합(월별 첫 레코드만 표시)은 후속 과제.
- **손익계산서 리디자인**(68d8c990): 연간|반기별|분기별 세그먼트. 반기/분기 비교 카드(합계+월평균+순이익률+월평균 기준 증감배지) + 기간별 결산보고서(개별 월+합계+월평균 열). 로마숫자 계정(Ⅰ~Ⅴ), 당기순이익 회계 이중선, 흑자/적자 스탬프, 과목 열 고정, tabular-nums, 이모지 제거. 셀 인라인 편집 유지. Playwright 3모드 스크린샷 검증.
...
| 카테고리 | 작업 내용 | 상태 |
|----------|----------|------|
| feat | 자재관리 그룹 메뉴 신설 (구매요청서·주거래처·품목·재고·영수증 보관함, 7개 서브메뉴 통합) | 완료 |
| feat | 구매요청서: 통합요청서 PDF·카톡 전송, 주문 단위(개/box/kg/g), 요청일/완료일, 날짜별 관리 | 완료 |
| feat | 영수증 AI 파이프라인: 촬영→gemma4:e4b 추출→자동분류→매입 반영, 카드/이체 중복 차단 | 완료 |
| feat | 품목: 중량·규격·수량·과세 자동분류·단가 이력(ProductPrice)·제품 사진 | 완료 |
| feat | 직원 앱 구매요청을 관리자 양식으로 통합 (requested_by 표기), 바텀내비 중앙 촬영 버튼 | 완료 |
| fix | 품목 business_id 누락(추가해도 안 보임), 영수증 detach FK 위반, 모바일 레이아웃 40여건 | 완료 |
| infra | 직원앱 Cloudflare Pages 배포 복구(wrangler 토큰), Play 개발자 계정 '셈하나' 생성 | 완료 |
| data | 영수증 42장 실등록(37건 287만원, 육안검수 정정), 거래처 정리(찬들농산·홍성상회·(주)유창 등) | 완료 |

### 세부 내용

- 자재관리 신설(5da7cb97)→PDF/카톡(963c9092)→영수증 AI(3ce6ac78)→주거래처(59e33ab0)→작성/관리 분리(20d8e5c2)
- 품목 상세·과세 자동분류·단가 이력 + bid 버그 픽스(c6f52d48), 중복 방지(834384ad), 단위 선택(383af9e1·07b3c950)
- 직원 앱 통합(5b480bf5), 날짜별 그룹(8baed46c·6285ad24), 모바일 전면 최적화+제품 사진(79bb48db)
- 영수증 42장 배치 임포트: gemma4:e4b 웜 3s/장, 연도 오독 13건·상호 오독 12건 육안 검수 정정, (주)유창 126만원 통합
